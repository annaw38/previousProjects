...
public class MyList<ElementType> implements ListADT<ElementType> {
    ...
    /**
     * Create a new mutable, but initially empty list.
    ...
    /**
     * Retrieves an element from a specific position with this list.
	...
	for(int i = index; i<size; i++)
	    array[i] = array[i];//change array[i+1] to array[i] so that
    // the higher elements aren't shifted down one position causing 
    //testRemovingFromEnds() to fail while the other 2 tests pass
