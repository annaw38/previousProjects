...
Pipe the output of the cat program as the input into the cut program.

We'll use two arguments to extract the data from column 5
...
and it will do that substitution for us.

Since we'd like this final argument to come from the output of the previous tail command, 
...
p1 (replacing/extending the provided cat command within this rule). 

After doing this, running "make p1" should produce the 
...
together again as we did in the previous section. 

The full command that we'll pipe the paste commands output into is: xargs 
