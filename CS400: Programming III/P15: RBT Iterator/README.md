...
During the search for the start point when initializing the stack, you can use the compareTo method of the start point's 
Comparable type to compare the start point to the first element in each duplicate list. You have access to the first element by calling the .iterator() method of KeyListInterface and then .next() on the returned list iterator.
