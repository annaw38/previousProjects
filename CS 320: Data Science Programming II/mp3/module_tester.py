...
def loans_test():
    ...
    #Additional Tests
    #Test 1: Ensure that Loan handles NA values for interest rate properly
    e = {"loan_amount": "5000", "property_value": "10000", "interest_rate": "NA", "applicant_age": "23", 
         "applicant_race-1": "42", "applicant_race-2": "", "applicant_race-3": "", "applicant_race-4": "", 
         "applicant_race-5": "","co-applicant_age": "9999", "co-applicant_race-1": "", "co-applicant_race-2": "", 
         "co-applicant_race-3": "", "co-applicant_race-4": "", "co-applicant_race-5": ""}
    loan = loans.Loan(e)
    assert loan.interest_rate == -1  # NA should convert to -1        
    loans_points += 1  
   
    # test 2: Ensure Applicant's lower_age method works with age ranges
    applicant = loans.Applicant("35-44", [])
    assert applicant.lower_age() == 35  
    loans_points += 1  
   
    # test 3: Ensure Loan's property value is initialized correctly
    d = {"loan_amount": "1000", "property_value": "200000", "interest_rate": "4.5", "applicant_age": "23",
         "applicant_race-1": "41", "applicant_race-2": "", "applicant_race-3": "", "applicant_race-4": "", 
         "applicant_race-5": "","co-applicant_age": "9999", "co-applicant_race-1": "", "co-applicant_race-2": "", 
         "co-applicant_race-3": "", "co-applicant_race-4": "", "co-applicant_race-5": ""}
    loan = loans.Loan(d)
    assert loan.property_value == 200000.0  
    loans_points += 1  
   
    # test 4: Ensure Applicant handles multiple races correctly
    codes = ["1", "2", "3"]  
    applicant = loans.Applicant("25-35", codes)
    expected_races = {race_lookup[code] for code in codes}
