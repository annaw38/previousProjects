...
defined by a row in the data set.
- After each country is represented by its feature vector (x1, . . . , x9), hierarchical agglomerative clustering (HAC) will be applied.
- This clustering approach will allow visualization of countries with similar socioeconomic conditions.
...
tiles labeled 1 through x and 9-x empty grids, where x is an integer between 1 and 8.
- Goal: rearrange the tiles so that they are in order and the empty places are at the bottom right.
